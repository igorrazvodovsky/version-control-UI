...
After this change, the backend HTTP adapter will be built on Oak instead of the hand-rolled router, and both the adapter and the API sync helpers will use Zod for input validation. A user can start the server and observe the same HTTP API behavior as before, but request parsing and validation will be simpler, more consistent, and easier to extend. The practical proof is that `deno test` still passes and basic HTTP calls (like creating a user and listing articles) behave the same while invalid JSON returns a structured 400 response.
...
- [x] (2026-02-02 18:33Z) Add Zod-based input parsing helpers and replace `syncs/app/helpers.ts` utilities.
...
The HTTP adapter now uses Oak and Zod-backed parsing for inputs, with per-endpoint schemas, and API sync helpers use Zod for validation utilities. Tests cover malformed JSON and still pass. The change improves request parsing consistency while preserving API behavior.

## Context and Orientation

The backend HTTP adapter is currently in `server.ts` and uses a custom router plus manual JSON parsing. It calls `createApp()` from `app.ts`, then triggers `API.request` and reads results from `API._get`. The API sync helpers in `syncs/app/helpers.ts` include ad-hoc parsing functions (`asRecord`, `getString`, `getStringArray`, etc.) that are used to validate request inputs inside syncs. The repository runs on Deno; server code imports from Deno std libraries and local TypeScript modules. The web frontend is in `apps/web` and is not part of this backend change.
...
- `syncs/app/helpers.ts`: input parsing helpers used by syncs.
- `syncs/app/*.ts`: API syncs that call the helpers.
...
4) Refactor `syncs/app/helpers.ts` to use Zod schemas while keeping current signatures.
...
- Core API flows (e.g., `POST /users`, `POST /version-control/init`) behave as in `server.test.ts`.
...
Expected test transcript excerpt (example):
    ...
    ok | http adapter: app + version control
...
- `syncs/app/helpers.ts` must continue exporting helper functions with the same names to avoid widespread refactors.
