...
## Overview
Add a dedicated Article view/edit page that integrates version control versioning. The first actual edit across a session (not just toggling edit mode), when no edit branch is selected, creates a new branch named `T-XXXXXXXX` from `main`; each edit branch has a single merge commit back to `main` (no intermediate commits). The right sidebar details panel shows the article's commit history plus branch details. Users can edit multiple articles within the same edit branch and commit. Discarding edits switches back to `main` only when the edit branch has no net changes; if `main` advanced, refresh the view and show a feedback toast.

## Problem Statement / Motivation
The web app currently lists articles but has no per-article view or editing workflow. version control branching and commit history exist in the backend, but the UI does not expose them. This feature makes article editing visible and traceable, aligning the UI with version control behavior.

## Proposed Solution
- Add a dynamic Article detail route (view + edit) and link to it from the articles table.
- On the first edit action across any article (e.g., first field change), when no edit branch is selected, create a version control branch with a unique `T-XXXXXXXX` name from `main`, switch current branch to it, and load the branch-scoped article as the working copy. Entering edit mode alone should not create a branch. If a user selects an existing in-progress edit branch, switch to it instead of creating a new branch. Creating a new branch while on a non-`main` branch must first switch back to `main` (no branches-of-branches).
...
- Edit flow: the first edit action in a session must (1) switch to `main` if needed, (2) create a branch from `main`, (3) switch current branch to it, (4) ensure the article exists on that branch (clone already handled in version control syncs), and (5) re-fetch article data on the new branch. Cache the base commit (from `main`) so discard can validate "no net changes" and the Branch tab can show a stable diff against the base. If an edit branch is selected, skip creation and just switch to it.
- Save flow: PUT /articles/:slug updates the working copy; POST /version-control/commits creates a single merge commit that merges the edit branch into `main`. After success, switch current branch to `main` and refresh article data. If there are no changes, skip the commit. If commit fails due to conflict or missing current branch, surface the error and keep the working copy.
...
- Frontend routing and state
  ...
  - Branch switching must handle in-progress form edits: either auto-persist form edits into the working copy before switching (preferred, version control-consistent) or block switching with a "save/discard" prompt.
...
- version control integration
  ...
  - After save, call `POST /version-control/commits` to merge the edit branch into `main`; the edit branch has a single commit. Commit message should be user-visible. Default message: "Update articles (N)" with optional per-article customization. Skip commit on no-op saves. After success, switch current branch back to `main`.
  ...
  - version control alignment check (docs/resources/version-control.md): this design keeps the “current branch” explicit, avoids detached head, enforces branches-from-main only, and merges edits back into `main` in a single commit.
- History API design
...
- Error handling
  - Surface 404/409/422 errors from version control endpoints in UI.
...
- [x] Making the first edit creates a new version control branch (named `T-XXXXXXXX`) from `main` and sets it as current; no branches-of-branches are created.
...
- [x] Saving edits updates the article and creates a single version control merge commit back to `main` with a non-empty message; if there are no changes, no commit is created.
...
- [x] Tests: add a version control history sync test and update any relevant web data-fetch tests.
...
## Dependencies & Risks
- version control branch state is global; switching branches in the UI affects all subsequent requests. Allowing multiple in-progress branches increases the need for clear branch selection and change list visibility.
...
- Prompt that worked: "Scan for existing Article, version control, and history-related syncs and list the relevant files."
- Tooling used: Codex CLI repo search and file inspection.
- Human review focus: verify version control branch switching does not leak across sessions; check error handling paths.
