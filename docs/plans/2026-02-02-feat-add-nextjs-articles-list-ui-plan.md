...
Build a new Next.js frontend (running on Deno) alongside the existing backend so a developer can view a public list of articles from the API (`GET /articles`). The UI should be minimal and focused on the articles list MVP.

## Problem Statement / Motivation

The repository already exposes HTTP endpoints via `server.ts`, but there is no frontend for quickly verifying or demoing the articles list. A lightweight UI will make development faster and provide a clear demo path.
...
- API list response is `{ articles: Article[], articlesCount: number }` and is already formatted by syncs.
...
- The backend is branch-scoped; it relies on `app.ts` initializing version control on startup.
...
- 📍 Local references:
  ...
  - `syncs/app/articles.ts:597` (list articles sync).
  - `syncs/app/articles.ts:723` (article list response formatting).
  - `app.ts:1` (engine wiring + version control init).
