...
### Step 4.5: Anti-OOP Drift Checks (Hard Gate)
Before finalizing the concept, explicitly check for object-oriented drift:
- **Set- and relation-first**: Does the state describe sets and relations (e.g., "a set of Users with ...") rather than a single object instance?
- **No instance-method thinking**: Are actions framed as operations over identifiers and sets, not as methods on a concrete object (avoid "user.updateName"-style thinking)?
- **No collection-objects**: Are you avoiding the creation of fake "manager" or "collection" concepts that exist only to provide set operations?
- **Composition via syncs**: If behavior spans multiple concepts, is it planned for synchronizations rather than cross-concept calls?

If any answer is "no", revise the design before proceeding.
...
6. **Avoid OO Drift**: Model purpose and relations explicitly; avoid object-centric design habits

## Resources

### references/

This skill includes detailed reference documentation that Codex can load as needed:
