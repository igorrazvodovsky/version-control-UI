...
- The design shows object-centric drift (concepts start to look like OO classes)
...
## Anti-OOP Drift Refactor Checks (Hard Gate)

Before choosing a move, explicitly check for object-centric drift:
- **Instance-method thinking**: Are actions framed as methods on an object instead of operations over identifiers and sets?
- **Single-instance state**: Does the state model a single object instance rather than a set/relationship structure?
- **Collection-object smell**: Are there fake manager/collection concepts that exist only to do set operations?

If any are true, refactor first to restore set- and relation-first modeling:
- **Split** object-shaped concepts into a true concept plus the missing relations/state.
- **Unify** or **Specialise** to model the correct abstraction level (avoid "UserCollection"-style concepts).
- **Loosen** synchronizations if coupling hides the real concept boundaries.
