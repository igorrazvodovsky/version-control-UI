...
- Is the design drifting into object-centric modeling?
...
#### Object-Centric Drift (Critical)
- **Symptom**: Actions are described like instance methods or the state models a single object instance
- **Example**: "User has methods updateName() and resetPassword()" without set/relations in state
- **Fix**: Reframe as operations over identifiers and sets; explicitly model sets and relations

#### Fake Collection Concepts
- **Symptom**: A "manager" or "collection" concept exists only to enable set operations
- **Example**: "UserCollection" that only adds/removes users
- **Fix**: Move set operations into the actual concept with proper state and actions
...
- [ ] **Object-centric framing**: Actions look like instance methods; state not set/relational
- [ ] **Collection-object smell**: Presence of manager/collection just to do set operations
...
- **Must not**: Represent a single instance instead of a set/relationship model
